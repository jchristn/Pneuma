...
A C# (Watson 7.1) backend + Postgres control plane + LiteGraph knowledge graph, with a document→graph→search ingestion pipeline built from DocumentAtom, RecallDB, and PolyPrompt (with in-process chunking via the `TextChunker` library and in-process embedding/summarization via PolyPrompt), plus three React/Vite dashboards (admin, subject, user). See `PNEUMA_PLAN.md` for the phased plan and progress checkboxes, and `REST_API.md` for the API.
