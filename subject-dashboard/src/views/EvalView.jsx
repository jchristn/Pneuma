...
function EvalProgressModal({ run, onClose, onFinished, onViewResults }) {
  ...
  return (
    <Modal isOpen onClose={onClose} title={t('eval.runProgress', 'Evaluation progress')} size="large"
      headerAction={<StatusPill status={status} />}>
      <div className="ev-prog">
        ...
        <div className="ev-modal-footer">
          {!terminal && (
            <button type="button" className="btn btn-danger" onClick={cancelRun} disabled={cancelling}>
              {cancelling ? t('common.loading', 'Working…') : t('eval.cancelRun', 'Cancel run')}
            </button>
          )}
          {terminal && finalRun && (
            <button type="button" className="btn btn-primary" onClick={() => onViewResults(finalRun)}>
              {t('eval.viewResults', 'View results')}
            </button>
          )}
          <button type="button" className="btn btn-secondary" onClick={onClose}>{t('common.close', 'Close')}</button>
        </div>
      </div>
    </Modal>
  );
...
function EvalDetailModal({ detail, onClose }) {
  ...
  return (
    <Modal isOpen onClose={onClose} title={t('eval.runResults', 'Run results')} size="fullscreen"
      headerAction={<StatusPill status={run.status} />}>
      <div className="ev-run-summary">
...
export default function EvalView() {
  ...
  const cancelRun = async (run) => { try { await apiClient.evalCancelRun(run.id); await load(); } catch (err) { setError(err.message); } };
  ...
  const runColumns = [
    { key: 'createdUtc', label: t('eval.started', 'Started'), render: (v) => formatDateTime(v) },
    { key: 'status', label: t('common.status', 'Status'), render: (v) => <StatusPill status={v} /> },
    { key: 'score', label: t('eval.score', 'Pass / Partial / Fail'), sortable: false,
      render: (_v, row) => <span>{row.passCount} / {row.partialCount} / {row.failCount} <span className="hd-muted">{t('eval.ofN', 'of {{n}}', { n: row.totalFacts })}</span></span> },
    { key: '_actions', label: t('common.actions', 'Actions'), sortable: false, className: 'actions-column',
      render: (_v, row) => (
        <ActionMenu actions={[
          { label: t('common.view', 'View'), onClick: () => openRun(row) },
          ...(isActive(row.status) ? [{ label: t('eval.cancelRun', 'Cancel run'), onClick: () => cancelRun(row) }] : []),
          { label: t('common.delete', 'Delete'), variant: 'danger', onClick: () => setConfirmRun(row) },
        ]} />
      ) },
  ];
  ...
  return (
    <div>
      ...
      <div className="filter-bar">
        ...
        {subjectId && (
          <button className="btn btn-primary" onClick={startRun} disabled={starting || facts.length === 0}
      ...
      {subjectId && (
        <>
          ...
          <DataTable columns={runColumns} data={runs} onRefresh={load} selection={selection} bulkBar={bulkBar}
            emptyTitle={t('eval.noRuns', 'No runs yet.')} />
        </>
      )}
      ...
      <ConfirmModal isOpen={!!confirmRun} onClose={() => setConfirmRun(null)} onConfirm={deleteRun}
        ...
        confirmLabel={t('common.delete', 'Delete')} isLoading={busy} />

      <ConfirmModal isOpen={bulkDeleteOpen} onClose={() => setBulkDeleteOpen(false)} onConfirm={bulkDelete}
        title={t('eval.deleteRun', 'Delete run')}
