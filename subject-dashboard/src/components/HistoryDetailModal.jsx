...
// A labelled roll-up card. `accent` shows a colored left rail.
...
// A single horizontal timing bar sized against the largest phase in the set.
...
export default function HistoryDetailModal({ detail, subjectName, onClose }) {
  ...
  return (
    <Modal isOpen={!!detail} onClose={onClose} title={t('history.detailTitle', 'Chat turn')} size="fullscreen">
      <div className="hd">
        {/* Identifiers */}
        <div className="hd-ids">
          ...
          {turn.subjectId && <div className="hd-id"><span className="hd-id-label">{t('history.subject', 'Subject')}</span><span className="hd-id-val">{subjectName ? subjectName(turn.subjectId) : turn.subjectId}</span><CopyButton value={turn.subjectId} /></div>}
