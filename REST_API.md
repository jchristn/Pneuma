...
| POST | `/v1.0/query` | Grounded answer. Body `{ question, maxResults?, subjectId? }` → `{ answer, sources: [node], grounded }`. When `subjectId` is set, retrieval is restricted to that subject's documents. |
