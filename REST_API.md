...
| POST | `/v1.0/query/stream` | Same as `/v1.0/query` but streamed over server-sent events (`metadata` / `delta` / `complete` / `error`). |
| POST | `/v1.0/chat/stream` | Multi-turn agentic chat over the corpus (the assistant may call read tools), streamed over SSE. Body `{ messages: [{ role, content }], maxResults? }`. |
