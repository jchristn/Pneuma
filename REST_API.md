...
| POST | `/v1.0/chat/stream` | Multi-turn agentic chat over the corpus (the assistant may call read tools), streamed over SSE. Body `{ messages: [{ role, content }], maxResults?, subjectId? }`. When `subjectId` is set, the search and grounded-answer tools are restricted to that subject. |
