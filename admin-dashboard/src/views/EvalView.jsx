  ...
  return (
    <div>
      ...
      <div className="filter-bar">
        ...
        {subjectId && (
          <button className="button-primary" onClick={startRun} disabled={running || facts.length === 0}
              ...
              <tbody>
                {runs.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td>
                      <button className="button-link" onClick={() => openRun(r)}>{t('common.view', 'View')}</button>
                      <button className="button-link ev-del" onClick={() => setConfirmRun(r)}>{t('common.delete', 'Delete')}</button>
                    </td>
                  </tr>
                ))}
      ...
      {detail && (
        <Modal title={t('eval.runResults', 'Run results')} size="wide" onClose={() => setDetail(null)}
          footer={<button type="button" className="button-secondary" onClick={() => setDetail(null)}>{t('common.close', 'Close')}</button>}>
