    ...
    public class ChatRoutes
    {
        ...
        private async Task ChatStreamAsync(HttpContextBase ctx)
        {
            ...
            using (lease)
            {
                ...
                try
                {
                    await _Chat.RunAsync(rc, request.Messages, max, request.SubjectId, request.ThreadId, request.MetadataFilter, (payload, isFinal, token) => sse.SendAsync(payload, isFinal, token), ctx.Token).ConfigureAwait(false);
                }
