namespace Pneuma.Server.Services
{
    ...
    public class IngestionStages
    {
        ...
        public async Task<MergeResult> MergeAsync(IngestionJob job, CandidateSubgraph subgraph, List<ExtractedCell> cells, CancellationToken token)
        {
            ...
            ApplyUserGraphMetadata(source, job);
        ...
        private static void ApplyUserMetadata(Dictionary<string, string> tags, IngestionJob job)
        {
            if (job.Labels != null)
            {
        ...
        /// <param name="job">The job carrying the operator labels/tags.</param>
        private static void ApplyUserGraphMetadata(GraphNode node, IngestionJob job)
        {
            if (job.Labels != null)
            {
                foreach (string label in job.Labels)
