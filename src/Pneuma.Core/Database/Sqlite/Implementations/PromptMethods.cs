namespace Pneuma.Core.Database.Sqlite.Implementations
{
    ...
    /// <summary>SQLite prompt methods.</summary>
    internal class PromptMethods : SqliteMethodsBase, IPromptMethods
    {
        ...
        public async Task<Prompt?> ReadByKeyAsync(string? tenantId, string key, CancellationToken token = default)
        {
            if (tenantId != null)
            {
                DataTable scoped = await Query(
                    "SELECT * FROM prompts WHERE promptkey = " + Sanitizer.Str(key) + " AND tenantid = " + Sanitizer.Str(tenantId) + " LIMIT 1;",
                    token).ConfigureAwait(false);
                if (scoped.Rows.Count > 0) return Map(scoped.Rows[0]);
            }

            DataTable global = await Query(
                "SELECT * FROM prompts WHERE promptkey = " + Sanitizer.Str(key) + " AND tenantid IS NULL LIMIT 1;",
                token).ConfigureAwait(false);
