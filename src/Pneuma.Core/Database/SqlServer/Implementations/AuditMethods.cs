namespace Pneuma.Core.Database.SqlServer.Implementations
{
    ...
    /// <summary>SQL Server audit record methods.</summary>
    internal class AuditMethods : SqlServerMethodsBase, IAuditMethods
    {
        ...
        public async Task<List<AuditRecord>> EnumerateAsync(string? tenantId, int maxResults, CancellationToken token = default)
        {
            ...
            if (tenantId == null)
            {
                sql = "SELECT TOP (" + maxResults.ToString(CultureInfo.InvariantCulture) + ") * FROM audit ORDER BY createdutc DESC;";
            }
            else
            {
                sql = "SELECT TOP (" + maxResults.ToString(CultureInfo.InvariantCulture) + ") * FROM audit WHERE tenantid = " + Sanitizer.Str(tenantId) + " ORDER BY createdutc DESC;";
            }
