namespace Pneuma.Core.Database.SqlServer.Implementations
{
    ...
    /// <summary>SQL Server persisted evaluation run methods.</summary>
    internal class EvalRunMethods : SqlServerMethodsBase, IEvalRunMethods
    {
        ...
        {
            DataTable table = await Query("SELECT TOP 1 * FROM evalruns WHERE status = 'Pending' ORDER BY createdutc ASC;", token).ConfigureAwait(false);
            if (table.Rows.Count == 0) return null;
