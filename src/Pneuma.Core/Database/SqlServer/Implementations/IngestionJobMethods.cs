namespace Pneuma.Core.Database.SqlServer.Implementations
{
    ...
    /// <summary>SQL Server ingestion job methods.</summary>
    internal class IngestionJobMethods : SqlServerMethodsBase, IIngestionJobMethods
    {
        ...
        {
            DataTable table = await Query(
                "SELECT TOP 1 * FROM ingestionjobs WHERE status = 'Queued' ORDER BY createdutc ASC;",
                token).ConfigureAwait(false);
            if (table.Rows.Count == 0) return null;
