namespace Pneuma.Core.Database.SqlServer.Implementations
{
    ...
    /// <summary>SQL Server per-subject prompt override methods.</summary>
    internal class SubjectPromptMethods : SqlServerMethodsBase, ISubjectPromptMethods
    {
        ...
        public async Task<SubjectPrompt?> ReadAsync(string tenantId, string subjectId, string promptKey, CancellationToken token = default)
        {
            DataTable table = await Query(
                "SELECT TOP 1 * FROM subjectprompts WHERE tenantid = " + Sanitizer.Str(tenantId) +
                " AND subjectid = " + Sanitizer.Str(subjectId) +
                " AND promptkey = " + Sanitizer.Str(promptKey) + ";",
                token).ConfigureAwait(false);
