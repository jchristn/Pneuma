namespace Pneuma.Core.Database.SqlServer.Implementations
{
    ...
    /// <summary>SQL Server request history methods.</summary>
    internal class RequestHistoryMethods : SqlServerMethodsBase, IRequestHistoryMethods
    {
        ...
        public async Task<RequestHistoryPage> EnumerateAsync(RequestHistoryFilter filter, CancellationToken token = default)
        {
            ...
            string listSql =
                ...
                " ORDER BY createdutc DESC OFFSET " + offset.ToString(CultureInfo.InvariantCulture) +
                " ROWS FETCH NEXT " + filter.PageSize.ToString(CultureInfo.InvariantCulture) + " ROWS ONLY;";
