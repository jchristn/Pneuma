namespace Pneuma.Core.Database.SqlServer.Implementations
{
    ...
    /// <summary>SQL Server role methods.</summary>
    internal class RoleMethods : SqlServerMethodsBase, IRoleMethods
    {
        ...
        public async Task<UserRole?> ReadByNameAsync(string? tenantId, string name, CancellationToken token = default)
        {
            if (tenantId != null)
            {
                DataTable scoped = await Query(
                    "SELECT TOP 1 * FROM userroles WHERE name = " + Sanitizer.Str(name) + " AND tenantid = " + Sanitizer.Str(tenantId) + ";",
                    token).ConfigureAwait(false);
                if (scoped.Rows.Count > 0) return Map(scoped.Rows[0]);
            }

            DataTable global = await Query(
                "SELECT TOP 1 * FROM userroles WHERE name = " + Sanitizer.Str(name) + " AND tenantid IS NULL;",
                token).ConfigureAwait(false);
